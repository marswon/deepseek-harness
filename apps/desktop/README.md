...
Release tags must be `v`-prefixed semver (`v0.1.0-rc.7`), not namespaced tags like `dsh-desktop-v*`: electron-updater's GitHub provider validates every tag in the releases feed with `semver.valid` and silently reports "No published versions on GitHub" when none parse. electron-updater only checks versions and downloads; the install handoff is app-owned: on Windows the shell force-stops stale installers in the updater pending directory (a zombie holds the per-app installer mutex and re-raises its own stale "cannot be closed" dialog on every later attempt), spawns the pending NSIS installer detached, and quits only after the spawn succeeds, while `build/installer.nsh` replaces the NSIS running-process gate with a prompt-free force kill. On macOS the app never updates in place: ad-hoc signed builds carry a per-binary cdhash designated requirement that Squirrel.Mac enforces against every update, so the updater downloads the dmg into `userData/updates/<version>` and opens it for a manual drag-replace.
