...
|---|---|---|---|---|
| dsh | Publish set: non-experimental `packages/*/*` + `apps/cli` + `apps/web` (`apps/desktop` ships as an Electron bundle, not an npm package); private experimental packages join only the shared version bump | one version for the publish set, private dsh packages, and workspace root, `0.0.x` | `dsh-v<version>` | `release.yml` |
