...
|---|---|---|---|---|
| dsh | 发布集：非 experimental 的 `packages/*/*` + `apps/cli` + `apps/web`（`apps/desktop` 以 Electron 壳分发，不发 npm 包）；私有实验性包仅加入共享版本 bump | 发布集、私有 dsh 包与 workspace 根共用一个 `0.0.x` | `dsh-v<版本>` | `release.yml` |
