...
- 开发模式用系统 Node 跑仓库构建产物；打包产物通过 `ELECTRON_RUN_AS_NODE=1` 把 Electron 二进制当 Node 复用。两者都传 `--expose-internals`，Cordis loader 走首选路径，不加载原生 `node-addon-require-builtin` 回退。node-pty 自带 N-API prebuild，ABI 在 Node 与 Electron 间稳定，打包无需原生重编（`npmRebuild: false`）。
...
- **安装包内附带独立 Node 二进制。** ABI 匹配最稳，但每个平台多约 50 MB 重复运行时；`ELECTRON_RUN_AS_NODE` 加 N-API prebuild 已经解决 ABI，这份重量买不到东西。
