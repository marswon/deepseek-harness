...
**首次使用弹窗行内渲染首启向导。** 适配器已挂载且处于活跃状态，其引用可解析、可写但尚未配置时，共用引导弹窗会打开三步向导；向导本身由[密钥引导 UI 笔记](2026-08-16-provider-key-guidance-ui.zh.md)持有。向导完成时的写入恰好是 Models 编辑器保存路径的凭据一半——密钥写入 profile 解析出的引用之下，不发出提供方 settings 变更；「跳过」只完成协调器当前这一轮。适配器缺失时仍跳过，因为浏览器不能挂载缺失的 Cordis 插件。
...
**在首次使用引导中单独实现 API key 表单**：不予采用，因为这会复制 Models 编辑器的 secret 草稿、校验、错误和已配置状态收敛。向导改为复用编辑器的实时检查与凭据写入，而非重新实现。
