...
**The prompt renders the first-run wizard inline.** A mounted, active adapter with a resolved, writable, unconfigured reference opens the three-step wizard inside the shared onboarding modal; [the key-guidance UI note](2026-08-16-provider-key-guidance-ui.md) owns that wizard. Its finish write is exactly the credential half of the Models editor’s save — the key under the profile’s resolved reference, no provider-settings mutation — and skipping completes only this coordinator pass. An absent adapter remains skipped because the browser cannot mount a missing Cordis plugin.
