...
**Run the runtime from a per-version staged copy under the desktop data root.** On first launch (and after every update, keyed by `app.getVersion()`), `stagePackagedRuntime` copies `resources/dsh-runtime` to `<userData>/runtimes/<version>` and the child launches from the copy. The install directory then hosts only Electron itself, which the NSIS close/kill path handles reliably; nothing the Harness runs can lock files the installer must replace. Staging is crash-safe — the copy lands in a `.staging-*` sibling and is renamed into place only after a completion marker is written, so a killed app never leaves a half-copied runtime; older versions and stale staging dirs are pruned after a successful stage. `resolveHarnessRuntime` wraps this so development mode passes through unchanged.
