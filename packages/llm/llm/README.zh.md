...
- **通过配置暴露并激活提供方**——适配器声明可配置提供方路由与 settings namespace，配置界面因此可以激活休眠提供方并编辑连接事实，无需重启；条目还可以携带提供方官方的 `consoleUrl`，因此界面只在适配器能给出目的地时才链接获取密钥动作。
- **发现与解析模型**——列出适配器公布的模型、询问端点它提供哪些模型，并解析某个精确模型的上下文窗口、输出默认值、推理（reasoning）强度与输入模态。点名适配器已认识路由的询问会由其注册表直接作答，不联网；`validate: true` 会撤掉这条捷径，适配器必须通过一次以给定密钥认证的实时往返作答——这正是界面检查密钥是否可用、而非适配器已有认知的方式。
