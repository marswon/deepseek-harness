...
function ok<T>(value: T): RpcResponse<T> {
  return { rpcId: `key-check-${nextRpc++}` as never, result: { ok: true, value } }
}
function fail<T>(message: string): RpcResponse<T> {
  return {
    rpcId: `key-check-${nextRpc++}` as never,
    result: { ok: false, error: { code: 'model-discovery-failed', message, details: { settingsNs: 'llm-deepseek' } } },
  }
}
...
describe('key check', () => {
  ...
  it('omits the key when the field is empty, letting the stored credential answer', async () => {
    ...
    await screen.findByText(en.keyCheckSuccess.replace('{count}', '3'))
    const payload = discover.mock.calls[0]?.[0] as Record<string, unknown>
  ...
  it('maps a refused key and an unreachable endpoint to actionable copy, else shows the raw message', async () => {
    const discover = vi.fn()
      .mockResolvedValueOnce(fail('https://api.deepseek.com/models answered 401; check the API key'))
      .mockResolvedValueOnce(fail('https://api.deepseek.com/models answered 403'))
      .mockResolvedValueOnce(fail('could not reach https://api.deepseek.com/models'))
      .mockResolvedValueOnce(fail('endpoint answered 500'))
...
describe('get-a-key guidance', () => {
  ...
  it('also renders in the credential-only posture the onboarding flow reuses', async () => {
    const discover = vi.fn(() => Promise.resolve(ok({ models: [{ id: 'a' }] })))
    const face = {
      llm: { discoverModels: discover },
      settings: { mutate: vi.fn() },
      credentials: {
        ...
        providers: vi.fn(() => Promise.resolve(ok({
          providers: [
            ...
            },
            {
              ...
              // Dormant: an active route with no profile would read as usable
              // through provider-native auth and lift the first-run posture.
              active: false,
