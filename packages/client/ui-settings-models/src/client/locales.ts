...
export const en = {
  ...
  keyCheckSuccess: 'Connected — {count} models available.',
  keyCheckInvalid: 'The API key is invalid or expired. Check it and paste it again.',
  keyCheckUnreachable: 'Could not reach the provider. Check your network or the API address.',
  keyCheckFinishHint: 'Run Check to verify the key first.',
  keyGuidanceDeepSeek: 'An API key identifies your account on the DeepSeek platform. Sign up, top up, and create a key there, then paste it here.',
  keyGuidanceGetDeepSeek: 'Get one from the DeepSeek platform',
  keyGuidanceGetConsole: 'Get an API key from the {name} console',
  ...
  onboardingStepLearn: 'Learn',
  onboardingStepConfigure: 'Configure',
  onboardingStepDone: 'Done',
  onboardingLearnWhat: 'An API key is a secret string the DeepSeek platform issues to your account: it identifies you and bills the calls you make. Pasting it here lets this app call models for you.',
  onboardingLearnHow: 'Sign up on the DeepSeek platform, top up your balance, then create a key on the API keys page and copy it.',
  onboardingNext: 'Next',
  onboardingBack: 'Back',
  onboardingSkip: 'Skip',
  onboardingFinish: 'Continue',
  onboardingDoneBody: 'All set: new conversations will use deepseek-v4-flash by default. You can switch models or add more providers anytime under Models settings.',
  onboardingStart: 'Start building',
...
export const zh: { [Key in keyof typeof en]: string } = {
  ...
  keyCheck: '检查',
  keyChecking: '检查中…',
  keyCheckSuccess: '连接成功，发现 {count} 个可用模型',
  keyCheckInvalid: '密钥无效或已过期，请检查后重新粘贴',
  keyCheckUnreachable: '无法连接到服务商，请检查网络或 API 地址',
  keyCheckFinishHint: '请先点击「检查」验证密钥',
  keyGuidanceDeepSeek: 'API Key 是 DeepSeek 开放平台发放的账户密钥，用于标识身份并结算调用费用。注册并充值后创建一个密钥，粘贴到这里即可。',
  keyGuidanceGetDeepSeek: '前往 DeepSeek 开放平台获取',
  keyGuidanceGetConsole: '前往 {name} 控制台获取 API Key',
  ...
  onboardingStepsLabel: '设置步骤',
  onboardingStepLearn: '了解',
  onboardingStepConfigure: '配置',
  onboardingStepDone: '完成',
  onboardingLearnWhat: 'API Key 是 DeepSeek 开放平台发放给你账户的一串密钥，用来标识身份并结算调用费用。把它粘贴到这里，应用才能替你调用模型。',
  onboardingLearnHow: '在 DeepSeek 开放平台注册并充值，进入「API Keys」页面创建一个密钥并复制。',
  onboardingNext: '下一步',
  onboardingBack: '上一步',
  onboardingSkip: '跳过',
  onboardingFinish: '完成',
  onboardingDoneBody: '一切就绪：新对话默认使用 deepseek-v4-flash 模型。之后可以随时在「模型」设置中更换模型或添加其他提供方。',
  onboardingStart: '开始使用',
  onboardingUseOther: '使用其他提供方',
