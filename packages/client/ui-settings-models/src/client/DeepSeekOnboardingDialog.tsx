...
export interface DeepSeekOnboardingInjected {
  ...
  /** The Host operations the credential write and key probe go through. */
...
export function DeepSeekOnboardingDialog(props: DeepSeekOnboardingDialogProps): ReactNode {
  ...
  const checkKey = (): void => {
    void keyCheck.check({
      ...
      /* v8 ignore next -- 检查 is disabled while the field is empty; probing the stored credential is the editor's posture, covered there */
  ...
  const start = async (): Promise<void> => {
    ...
    try {
      const stored = await operations.storeCredential(
        refFor(schema, namespace, row.entry.settingsPath, row.entry.provider),
        keyValue,
      )
      if (stored !== undefined) {
        setFailure(stored)
  ...
  return (
    <OnboardingModal title={t('onboardingTitle')} focusTitle={step !== 2}>
      <ol className={styles.steps} aria-label={t('onboardingStepsLabel')}>
        {[t('onboardingStepLearn'), t('onboardingStepConfigure'), t('onboardingStepDone')].map((label, index) => {
