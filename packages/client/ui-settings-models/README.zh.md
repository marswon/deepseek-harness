...
声明完成后，DeepSeek 步骤会从同一个 Models 联接快照得出首次运行就绪状态。只要用户已经能触达**任何**一个提供方，它就直接完成而不渲染——已注册且其具名凭据引用已存储的路由（包括来自启动环境且只读的凭据），或 profile 根本不指名引用、因而走原生认证的路由。只有二者皆无的用户才会被要求填写 DeepSeek 官方密钥。适配器已挂载且活跃、引用可写但尚未配置时，共用引导弹窗内会打开一个三步向导：「了解」解释什么是 API Key 并链接到 DeepSeek 控制台；「配置」粘贴密钥并运行与编辑器相同的实时校验——提供方接受密钥前「完成」保持禁用，「跳过」保留原"稍后配置"语义；最后一步说明默认使用 deepseek-v4-flash，并在「开始使用」时写入密钥，每一步底部的「使用其他提供方」链接可离开向导前往 Models 页。`credentials.set` 仍是唯一的 secret 写入，且不会改变提供方设置。跳过只完成协调器当前这一轮。适配器缺失、路由不活跃、联接失败、部署只读或设置／凭据能力不可用时，该步骤不渲染并直接完成；Models 页仍是诊断界面。
