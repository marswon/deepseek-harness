...
1. **Roster** — the composed profile's plugin rows (standard YAML parse under Include's dialect, `!!js` intact), plus the rows of every config tree the CLI declares in its `package.json` `dsh.configTrees` (agent presets), materialized as a Node-style dependency closure. A row name with a scope or path separator is a module specifier outright; a bare name (the web profile's `dshmarket`) joins the roster only when it resolves as a package — through the workspace index, the repository root, or a caller-passed install anchor such as the `dsh` CLI package directory (`rosterResolveFrom`), which pnpm never hoists to the root — so preset metadata stays out. External peer edges never bind the worker; workspace peers stay on the chain.
...
3. **Reachability sweep** — the runtime loader's own resolution walks from the export faces of every materialized roster package (workspace and vendored packages are addressed by constructed name at runtime; an external seed such as `dshmarket` is named by a Loader row directly) plus the worker assembly's seeds (`IMAGE_ENTRY_SEEDS`), lowering each reached module to the wrapper contract at pack time. The transform reports statically named imports, re-exports, and dynamic imports; calls through `require`; and module-scope direct calls of the form `createRequire(import.meta.url)('pkg')` through a named import from `node:module` or `module`, including an import alias. Page assets (`lib/client.js` behind `./client` exports) ship verbatim; an unresolvable request from our own code fails the pack, third-party ones are tolerated to fail loud at require time.
